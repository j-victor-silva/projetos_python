...
def password(tam=8):
    ...
    for i in range(tam):
        ...
        senha += s.ascii_letters[valor_letters] + s.digits[valor_digits] + s.punctuation[valor_punctuation]
...
while True:
    ...
    topico = input('Site ou programa que a senha será gerada: ')
    file.write(f'\nSite ou Programa: {topico}\n')
    user = input('\nUsuário ou Email que vai receber a nova senha: ')
    file.write(f'\n\tUsuário ou Email: {user}\n')
    inp_senha = input('\nDigite o tamanho que terá sua senha, caso não digite nada ou digite um valor menor que 8 a senha terá um tamanho de 8 caracteres: ')
    
    if inp_senha == "" or int(inp_senha) < 8:
